...
export default function BottomNav() {
  ...
  return (
    <nav 
      className="bottom-nav-container fixed bottom-0 left-0 right-0 z-40 glass-card border-t border-b-0 border-l-0 border-r-0 pb-nav-ios desktop-hide-bottom-nav bg-slate-950/95 backdrop-blur-xl shadow-2xl"
      style={{
        paddingLeft: 'max(0.5rem, env(safe-area-inset-left, 0px))',
        paddingRight: 'max(0.5rem, env(safe-area-inset-right, 0px))'
      }}
    >
      <div className="bottom-nav-inner grid grid-cols-5 items-center h-[50px] landscape-compact-nav w-full px-1 sm:px-4">
        {navItems.map((item) => {
          ...
          return (
            ...
            >
              ...
              <span className="bottom-nav-label text-[10px] font-medium tracking-tight">{item.label}</span>
            </Link>
          )
        })}
      </div>
