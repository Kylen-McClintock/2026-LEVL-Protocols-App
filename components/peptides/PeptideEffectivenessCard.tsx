...
export default function PeptideEffectivenessCard({
  ...
  return (
    <div className="rounded-3xl bg-gradient-to-b from-slate-900 via-slate-900 to-black border border-cyan-500/30 overflow-hidden shadow-2xl backdrop-blur-xl transition-all">
      {/* Clickable Header with Adherence Badge & Dropdown Arrow */}
      <button
        type="button"
        onClick={() => setIsExpanded(!isExpanded)}
        className="w-full flex items-center justify-between p-4 sm:p-5 text-left hover:bg-cyan-950/20 transition-colors cursor-pointer"
      >
        ...
        <div className="flex items-center gap-3 shrink-0 ml-3">
          ...
          <div className="p-1.5 rounded-xl bg-cyan-500/10 border border-cyan-500/20 text-cyan-300 hover:bg-cyan-500/20 transition-all">
            <ChevronDown size={18} className={`transition-transform duration-200 ${isExpanded ? 'rotate-180 text-cyan-400' : 'text-slate-400'}`} />
          </div>
        </div>
      </button>
      ...
      {isExpanded && (
        <div className="p-5 pt-0 space-y-5 border-t border-white/5 animate-in fade-in slide-in-from-top-2">
          {/* Progress Bar for Initial Calibration */}
          {isCollectingBaseline && (
            <div className="p-3.5 rounded-2xl bg-black/50 border border-white/5 space-y-2 mt-4">
          ...
          <div className={`space-y-3 ${!isCollectingBaseline ? 'mt-4' : ''}`}>
