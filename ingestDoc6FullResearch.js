...
const doc6Path = '/Users/kylenmcclintock/.gemini/antigravity-ide/brain/93afc293-0e8a-432f-aa9c-09556efd4195/.system_generated/steps/9000/content.md';
const doc6 = fs.readFileSync(doc6Path, 'utf8');
...
const doc6Sections = [
  { id: 'bfr_training', pattern: 'bfr_training — BFR Training', keywords: ['bfr_training', 'bfr'] },
  { id: 'running', pattern: 'running — Running', keywords: ['running'] },
  { id: 'zone_2_cardio', pattern: 'zone_2_cardio — Zone 2', keywords: ['zone_2', 'zone 2'] },
  { id: 'strength-training', pattern: 'strength-training — Strength Training', keywords: ['strength-training', 'strength_training'] },
  { id: 'optic_flow', pattern: 'optic_flow — Optic Flow', keywords: ['optic_flow', 'optic flow'] },
  { id: 'recovery-techniques', pattern: 'recovery-techniques — Recovery', keywords: ['recovery-techniques', 'active_recovery'] },
  { id: 'dark-cool-sleep-environment', pattern: 'dark-cool-sleep-environment', keywords: ['dark-cool-sleep', 'sleep_environment'] },
  { id: 'walker_metabolic_alcohol_cutoff', pattern: 'walker_metabolic_alcohol_cutoff', keywords: ['alcohol_cutoff', 'metabolic_cutoff'] },
  { id: 'walker_sleep_triad_supplement', pattern: 'walker_sleep_triad_supplement', keywords: ['walker_sleep_triad', 'sleep_triad'] },
  { id: 'evening-screen-time-reduction', pattern: 'evening-screen-time-reduction', keywords: ['screen_time', 'screen-time'] },
  { id: 'walker_melatonin_dimming', pattern: 'walker_melatonin_dimming', keywords: ['melatonin_dimming', 'blue_dimming'] },
  { id: 'walker_65f_thermal_drop', pattern: 'walker_65f_thermal_drop', keywords: ['65f', 'thermal_drop'] },
  { id: 'mouth_taping', pattern: 'mouth_taping — Sleep Mouth Taping', keywords: ['mouth_taping', 'mouth taping'] }
];

const mods = JSON.parse(fs.readFileSync('all_modalities.json', 'utf8'));
const doc6VideoMap = {};

doc6Sections.forEach((sec, idx) => {
  const startIdx = doc6.indexOf(sec.pattern);
  const nextSec = doc6Sections[idx + 1];
  const endIdx = nextSec ? doc6.indexOf(nextSec.pattern, startIdx) : doc6.length;

  if (startIdx !== -1) {
    const htmlChunk = doc6.substring(startIdx, endIdx !== -1 ? endIdx : doc6.length);
    ...
    mods.forEach((m, mIdx) => {
      ...
      if (sec.keywords.some(kw => nameStr.includes(kw))) {
        ...
        if (videoId) {
          doc6VideoMap[m.id] = { videoId, startSeconds, title: videoTitle };
        }
        matchCount++;
      }
    });

    console.log(`DOC6 SECTION: ${sec.id} | Text: ${text.length} chars | Table: ${Boolean(table)} | Video: ${videoId || 'None'} @ ${startSeconds}s | Modalities Updated: ${matchCount}`);
  }
});

fs.writeFileSync('all_modalities.json', JSON.stringify(mods, null, 2));
console.log('Successfully saved updated all_modalities.json with Doc 6 research data.');
...
Object.entries(doc6VideoMap).forEach(([id, info]) => {
  const keyQuote = `'${id}':`;
  const keyPlain = `${id}:`;
...
console.log('Successfully updated modalityVideos.ts with Doc 6 videos.');
...
async function syncDoc6ToSupabase() {
  console.log('Syncing updated Doc 6 modalities to Supabase Cloud DB...');
  let syncCount = 0;
  for (const m of mods) {
